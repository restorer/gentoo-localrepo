...
DESCRIPTION="A complete toolset for C and C++ development (with bundled tools included)"
...
src_install() {
	...
	insinto "${dir}"
	doins -r *
	fperms 755 "${dir}"/bin/{clion.sh,clang/linux/clang{d,-tidy},cmake/linux/bin/c{cmake,make,pack,test},gdb/linux/bin/g{core,db,db-add-index,dbserver},lldb/linux/bin/ll{db,db-argdumper,db-server},lldb/linux/bin/LLDBFrontend}
	...
	echo "fs.inotify.max_user_watches = 524288" > "${D}/usr/lib/sysctl.d/30-clion-bundled-inotify-watches.conf" || die
}
